...
其他插件可通过 Bukkit ServicesManager 获取 `RelayRaceApi`，调整场上玩家的剩余游玩时间。所有方法须在主线程调用。

```java
import top.syshub.relayrace.common.api.RelayRaceApi;

RelayRaceApi api = RelayRaceApi.get(); // 或 ((RelayRacePlugin) plugin).getApi()
if (api != null && api.isGameRunning()) {
    api.addRemainingSeconds(30);        // 增加 30 秒
    api.subtractRemainingSeconds(10);   // 减少 10 秒
...
| 方法                            | 说明                                                |
|---------------------------------|-----------------------------------------------------|
| `isGameRunning()`               | 当前是否有进行中的对局                              |
| `getActivePlayer()`             | 当前活跃玩家，无则 `null`                           |
| `getRemainingSeconds()`         | 本轮剩余秒数（未开局为 0）                          |
| `addRemainingSeconds(int)`      | 增加剩余时间（秒，须为正）                          |
| `subtractRemainingSeconds(int)` | 减少剩余时间（秒，须为正）；减到 0 时立即切换下一位 |

`addRemainingSeconds` / `subtractRemainingSeconds` 在未开局或参数 `<= 0` 时返回 `false`。
